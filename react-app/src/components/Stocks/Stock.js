            ...
            <div className="buy-stock-div1">
              <form  className='buy-stock-form' onSubmit={submitBuy}>
                ...
                <div className="assestSymboldiv">
                  <span className="turnOrange spanBuy" onClick={showBuyForm}> Buy {stock["symbol"]} </span>
                  <span className="turnOrange spanSell" onClick={showSellForm}> Sell {stock["symbol"]} </span>
                </div>
                <div>
                ...
                <div>
                  {showSell ? <span className="InvestLabel ES1">Estimated Credit</span> : <span className="InvestLabel ES1">Estimated Cost</span> }
                  <span className="InvestLabel ES2"> ${(share * stock.latestPrice).toLocaleString('en')} </span>
                </div>
                  {showSell ? <div><button className="reviewOrder" onClick={purchaseStock}>Review Order Sell</button></div> : <div><button className="reviewOrder" onClick={purchaseStock}>Review Order Buy</button></div>}
                  {showSell ? <div><span className="InvestLabel il5">1 Share Available-Sell All</span></div> : <div><span className="InvestLabel il5">${user["buying_pwr"].toLocaleString('en')} buying power available</span></div>}
